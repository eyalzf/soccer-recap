...
export const LEAGUE_SEARCH_PLANS: Record<string, LeagueSearchPlan> = {
  ...
  'champions-league': {
    ...
    bulk: [
      ...
      { handle: 'Atleti', label: 'Atletico Madrid' },
  ...
  'premier-league': {
    preferred: [],
    bulk: [
      { handle: 'SkySportsPL', label: 'Sky Sports Premier League' },
      { handle: 'mancity', label: 'Man City' },
      { handle: 'manutd', label: 'Man Utd' },
      { handle: 'Arsenal', label: 'Arsenal' },
      { handle: 'LiverpoolFC', label: 'Liverpool' },
      { handle: 'chelseafc', label: 'Chelsea' },
      // TODO(verify): Tottenham (handle unconfirmed); Leeds/Newcastle and
      // other club channels post per-game highlights — add after verifying
      // handles + recency.
    ],
    fallbackLangs: ['en'],
  },
